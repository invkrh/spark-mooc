...
def simpleTokenize(string):
    ...
    """
    return [tkn for tkn in re.split(split_regex, string.lower()) if tkn != '' and tkn != ' ']
...
def tokenize(string):
    ...
    """
    return [tkn for tkn in simpleTokenize(string) if tkn not in stopwords]
...
def tf(tokens):
    ...
    return dict([(i, tokens.count(i) / size) for i in tokens])
...
def idfs(corpus):
    ...
    uniqueTokens = corpus.flatMap(lambda (id, tkns): set(tkns))
    tokenCountPairTuple = uniqueTokens.map(lambda tkn: (tkn, 1))
...
def tfidf(tokens, idfs):
    ...
    """
    tfs = tf(tokens)
    tfIdfDict = dict([(key, value * idfs[key]) for key, value in tfs.iteritems()])
    return tfIdfDict
...
# TODO: Replace <FILL IN> with appropriate code
sims = similaritiesBroadcast.map(lambda (g, a, score): (a + ' ' + g, score))

trueDupsRDD = (sims
               .join(goldStandard)
               .values()
               .map(lambda (score, gold): score))
...
nonDupsRDD = (sims
              ...
              .values()
              .filter(lambda (score, gold): gold == None)
              .map(lambda (score, gold): score))
...
amazonWeightsRDD = amazonFullRecToToken.map(lambda (id, tkns): (id, tfidf(tkns, idfsFullBroadcast.value)))
googleWeightsRDD = googleFullRecToToken.map(lambda (id, tkns): (id, tfidf(tkns, idfsFullBroadcast.value)))
...
commonTokens = (amazonInvPairsRDD
                ...
                .groupByKey()
                .cache())
commonTokens.setName("commonTokens")
...
similaritiesFullRDD = (commonTokens
                       ...
                       .cache())
similaritiesFullRDD.setName("similaritiesFullRDD")
