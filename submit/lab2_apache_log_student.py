...
daysWithHosts = dailyHosts.map(lambda (day, cnt): day).collect()
hosts = dailyHosts.map(lambda (day, cnt): cnt).collect()
...
daysWithErrors404 = errDateSorted.map(lambda (day, cnt): day).collect()
errors404ByDay = errDateSorted.map(lambda (day, cnt): cnt).collect()
...
hoursWithErrors404 = hourRecordsSorted.map(lambda (hour, cnt): hour).collect()
errors404ByHours = hourRecordsSorted.map(lambda (hour, cnt): cnt).collect()
