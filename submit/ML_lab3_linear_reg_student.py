...
def parsePoint(line):
    ...
    """
    rawArr = line.split(',')
    return LabeledPoint(rawArr[0], rawArr[1:])
...
def calcRMSE(labelsAndPreds):
    ...
    """
    sumError = labelsAndPreds.map(lambda (label, pred): squaredError(label, pred)).sum()
    cntError = labelsAndPreds.count()
    return np.sqrt( sumError / float(cntError))
...
def twoWayInteractions(lp):
    ...
    twoWayFeatures = [x*y for (x, y) in itertools.product(features, features)]
    finalFeatures = np.hstack((features, twoWayFeatures))
    return LabeledPoint(label, finalFeatures)
