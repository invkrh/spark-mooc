...
rawTrainData, rawValidationData, rawTestData = rawData.randomSplit(weights, 42)
# Cache the data
rawTrainData.cache()
...
def parseOHEPoint(point, OHEDict, numOHEFeats):
    ...
    label = strArr[0]
    features = strArr[1:]
    keys = [(i, f) for i, f in enumerate(features)]
    return LabeledPoint(label, oneHotEncoding(keys, ctrOHEDict, numOHEFeats))
...
logLossTrBase = OHETrainData.map(lambda lp: computeLogLoss(classOneFracTrain, lp.label)).sum() / float(OHETrainData.count())
...
def evaluateResults(model, data):
    ...
    """
    return data.map(lambda lp: computeLogLoss(
            getP(lp.features, model.weights, model.intercept),
            lp.label)).sum() / float(data.count())
...
# TODO: Replace <FILL IN> with appropriate code
logLossValBase = OHEValidationData.map(lambda lp: computeLogLoss(classOneFracTrain, lp.label)).sum() / float(OHEValidationData.count())
...
def parseHashPoint(point, numBuckets):
    ...
    label = point[0]
    features = point[1:]
    return LabeledPoint(label, SparseVector(numBuckets, hashFunction(numBuckets, parsePoint(point), False)))
...
def computeSparsity(data, d, n):
    ...
    """
    return data.map(lambda lp: len(lp.features.indices) / float(d)).sum() / n
...
logLossTestBaseline = hashTestData.map(lambda lp: computeLogLoss(classOneFracTrain, lp.label)).sum() / float(hashTestData.count())
