...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    ratingSum = float(sum(IDandRatingsTuple[1]))
    ratingSize = len(IDandRatingsTuple[1])
    return (movieID, (ratingSize, ratingSum / ratingSize))
...
movieIDsWithRatingsRDD = (ratingsRDD
                          .groupBy(lambda (u, m, rt): m)
                          .mapValues(lambda it: [tpl[2] for tpl in it]))
...
movieNameWithAvgRatingsRDD = (moviesRDD
                              ...
                              .map(lambda (movieID, (title, (cnt, avg))): (avg, title, cnt)))
...
movieLimitedAndSortedByRatingRDD = (movieNameWithAvgRatingsRDD
                                    .filter(lambda (avg, name, cnt): cnt > 500)
...
def computeError(predictedRDD, actualRDD):
    ...
    predictedReformattedRDD = predictedRDD.map(lambda (u, m, rt): ((u, m), rt))

    # Transform actualRDD into the tuples of the form ((UserID, MovieID), Rating)
    actualReformattedRDD = actualRDD.map(lambda (u, m, rt): ((u, m), rt))
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                        ...
                        .values()
                        .map(lambda (pre, act): math.pow(pre - act, 2)))
...
validationForPredictRDD = validationRDD.map(lambda (u, m, rt): (u, m))
...
testForPredictingRDD = testRDD.map(lambda (u, m, rt): (u, m))
...
trainingAvgRating = trainingRDD.map(lambda (u, m, rt): rt).sum() / trainingRDD.count()
print 'The average rating for movies in the training set is %s' % trainingAvgRating

testForAvgRDD = testRDD.map(lambda (u, m, rt): (u, m, trainingAvgRating))
...
myUnratedMoviesRDD = (moviesRDD
                      .filter(lambda (mid, title): mid not in set([x[1] for x in myRatedMovies]))
                      .map(lambda (mid, title): (0, mid)))
...
movieCountsRDD = movieIDsWithAvgRatingsRDD.map(lambda (mid, (cnt, avg)): (mid, cnt))

# Transform predictedRatingsRDD into an RDD with entries that are pairs of the form (Movie ID, Predicted Rating)
predictedRDD = predictedRatingsRDD.map(lambda (u, m, rt): (m, rt))
...
ratingsWithNamesRDD = (predictedWithCountsRDD
                       .filter(lambda (mid, (rt, cnt)): cnt > 75)
                       .join(moviesRDD)
                       .map(lambda (mid, ((rt, cnt), title)): (rt, title, cnt)))
